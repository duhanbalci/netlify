        ...
        <q-card-section>
          ...
          <div class="q-mb-md" v-if="dialogProduct.addable">
            <div class="text-subtitle2">Eklenebilir Malzemeler</div>
              <q-chip :color="dialogProduct.addable[i].value ? 'orange' : 'white'" :selected.sync="dialogProduct.addable[i].value" v-for="(v, i) in dialogProduct.addable" :key="v.name">
                {{v.name}}
              </q-chip>
          </div>
          <div class="q-mb-md" v-if="dialogProduct.removable">
            ...
            <q-chip :color="dialogProduct.removable[i].value ? 'orange' : 'white'" :selected.sync="dialogProduct.removable[i].value" v-for="(v, i) in dialogProduct.removable" :key="v.name">
                {{v.name}}
              </q-chip>
          </div>
...
export default {
  ...
  methods: {
    openDialog (obj) {
      ...
      this.dialogProduct.addable = this.dialogProduct.addable.map(v => ({ name: v, value: false }))
      this.dialogProduct.removable = this.dialogProduct.removable.map(v => ({ name: v, value: false }))
    },
    addToCart () {
    ...
    async fetchMenu () {
      const res = await this.$axios.get(`${URL}/product/menu/1`)
